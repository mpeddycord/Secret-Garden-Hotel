...
import BookRoomsCta from "@/src/components/BookRoomsCta/BookRoomsCta";
   ...
   if (error) {
    ...
    const handleBookNowClick = async () => {
        if (!checkInDate || !checkOutDate) {
            return toast.error("Please provide check in and check out dates");
        }
        if (checkInDate > checkOutDate) {
            return toast.error("Please choose a valid check in date");
        }
        ...
        try {
            const {data: stripeSession} = await axios.post('/api/stripe', {checkInDate, checkOutDate, adults, children: numChildren, numberOfDays, hotelRoomSlug});
        ...
        <div className="container mx-auto mt-20">
            <div className="md:grid md:grid-cols-12 gap-10 px-3">
                    ...
                    <div>
                        ...
                        <div className="mb-11">
                            ...
                            <div className="grid grid-cols-2">
                                {room.offeredAmenities.map(amenity => <div key={amenity._key} className="flex items-center md:my-0 my-1">
                ...
                <div className="md:col-span-4 rounded-xl shadow-lg dark:shadow dark:shadow-white sticky top-10 h-fit overflow-auto">
                    <BookRoomsCta  discount={room.discount} price={room.price} specialNote={room.specialNote}  checkInDate={checkInDate} setCheckInDate={setCheckInDate} checkOutDate={checkOutDate} setCheckOutDate={setCheckOutDate} calcMinCheckoutDate={calcMinCheckoutDate} adults={adults} setAdults={setAdults} numChildren={numChildren} setNumChildren={setNumChildren} isBooked={room.isBooked} handleBookNowClick={handleBookNowClick}/>
                </div>
            </div>
        </div>
    </div>
