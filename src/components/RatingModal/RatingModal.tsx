    ...
    <div className={`fixed z-[61] inset-0 flex items-center justify-center ${isOpen ? "opacity-100 pointer-events-auto" : 'opacity-0 pointer-events-none'} `}>
        <div className="bg-white w-96 p-4 rounded-lg shadow-lg">
            ...
            <div className="mb-4">
                ...
                <textarea rows={4} value={ratingText} onChange={e => setRatingText(e.target.value)} className="w-full px-2 py-3 border rounded-md "></textarea>
            </div>
            <div className="flex justify-end">
                <button className="px-4 py-2 bg-primary text-white rounded-md" onClick={reviewSubmitHandler} disabled={isSubmittingReview}>
                    {isSubmittingReview ? "Submitting": "Submit"}
                </button>
                <button className="ml-2 px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400" onClick={toggleRatingModal}>Cancel</button>
            </div>
        </div>
    </div>
  );
